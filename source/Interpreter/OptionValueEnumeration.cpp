...
size_t OptionValueEnumeration::AutoComplete(CommandInterpreter &interpreter, CompletionRequest &request) {
  ...
  if (!request.GetCursorArgumentPrefix().empty()) {
    ...
    for (size_t i = 0; i < num_enumerators; ++i)
      request.GetMatches().AppendString(m_enumerations.GetCStringAtIndex(i).GetStringRef());
  }
