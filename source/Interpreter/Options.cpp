...
bool Options::HandleOptionCompletion(CompletionRequest &request, OptionElementVector &opt_element_vector,
  ...
  for (size_t i = 0; i < opt_element_vector.size(); i++) {
    ...
    if (opt_pos == request.GetCursorIndex()) {
      ...
      if (opt_defs_index != -1) {
        HandleOptionArgumentCompletion(subrequest,
            opt_element_vector,
            interpreter);
...
bool Options::HandleOptionArgumentCompletion(
  ...
  if (enum_values != nullptr) {
    ...
    std::string match_string(request.GetParsedLine().GetArgumentAtIndex(opt_arg_pos),
                             request.GetParsedLine().GetArgumentAtIndex(opt_arg_pos) + request.GetCursorCharPosition());
  ...
  if (completion_mask & CommandCompletions::eSourceFileCompletion ||
    ...
    for (size_t i = 0; i < opt_element_vector.size(); i++) {
      ...
      if (cur_opt_name && strcmp(cur_opt_name, "shlib") == 0 &&
          cur_arg_pos != -1) {
        const char *module_name = request.GetParsedLine().GetArgumentAtIndex(cur_arg_pos);
        if (module_name) {
